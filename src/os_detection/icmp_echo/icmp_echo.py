...
def get_ie(responses):
    ...
    # first probe only and that is weird CHECK
    params['CD'] = get_ie_cd(responses)
...
def send_icmp_probes(dst, dport):
    ...
    send(IP(dst=dst, flags='DF', tos=0, id=ip1_id) /
         ICMP(code=9, seq=295, id=icmp1_id) / payload1)
    send(IP(dst=dst, tos=4, id=ip2_id) /
         ICMP(code=0, seq=296, id=icmp2_id) / payload2)
